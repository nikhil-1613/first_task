...
function SingleProductPage() {
  ...
  if (!["architect", "client"].includes(role)) {
    toast.warning("Login as client or architect to see prices");
    return;
  }
  ...
  if (!item._id || !user?._id) {
    ...
    console.error("Missing IDs:", { item, user });
    return;
  }
  ...
  try {
    ...
    await addToCartAPI({ userId: user._id, item }, token);
  ...
  const handleToggleFavourite = async () => {
    if (!isLoggedIn) {
      toast.warning("Login to manage favourites");
      return navigate("/login");
    }
    ...
    <div className="bg-white" ref={mainRef}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 px-6 py-10 max-w-7xl mx-auto">
        ...
        <div className="space-y-4">
          ...
          <p className="text-xl font-semibold text-red-500">Price: ₹{finalPrice}</p>
          {product.oldPrice && (
          ...
          <div className="pt-6 border-t text-sm text-gray-700 space-y-1">
            ...
            <p className="flex items-center gap-3">
              ...
              <span className="flex gap-3">
                <FaShare />
                <FaHeart onClick={handleToggleFavourite} />
              </span>
            </p>
          </div>
        </div>
