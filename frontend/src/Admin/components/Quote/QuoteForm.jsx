...
const statusOptions = ["Approved", "In Review", "Send", "Shortlisted", "Rejected"];
const teamMembers = ["R", "V", "T", "B"]; // Assigned initials
...
function QuoteForm() {
  ...
  const toggleAssigned = (initial) => {
    const updated = formData.assigned.includes(initial)
      ? formData.assigned.filter((i) => i !== initial)
      : [...formData.assigned, initial];
    setFormData({ ...formData, assigned: updated });
  };
          ...
          <div className="bg-white border rounded-xl shadow p-6 w-full">
            <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {[
                ...
                { label: "Lead ID", name: "leadId" },
                { label: "Budget", name: "budget" },
                { label: "Contact", name: "contact" },
                { label: "Quote Amount", name: "quoteAmount" },
                { label: "City / Area", name: "city" },
              ].map((field) => (
                <div key={field.name}>
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {teamMembers.map((initial) => (
                    <label key={initial} className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={formData.assigned.includes(initial)}
                        onChange={() => toggleAssigned(initial)}
                      />
