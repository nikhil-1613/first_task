    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex flex-col flex-1 overflow-hidden bg-gray-100">
        ...
        <div className="bg-white px-4 py-1 border-b text-sm text-gray-700">
          ...
          <span
            className={`font-semibold ${isHuelip ? "text-green-600" : "text-gray-500"}`}
          >
            ...
            <React.Fragment key={section}>
              <button
                ...
                className={`text-sm font-semibold ${
                  activeSection === section ? "text-red-800 underline" : "text-gray-700"
                }`}
              >
                {section}
              </button>
              {index < sections.length - 1 && <span className="text-red-800">|</span>}
            </React.Fragment>
          ))}
      ...
      >
        ...
        <div className="border-2 border-black rounded-2xl px-4 py-3 space-y-4">
          {["Bedroom", "Living Room", "Toilet", "Pantry", "Store", "Balcony"].map(
            (space) => (
                ...
                {loadingQuotes ? (
                  ...
                  quotes.map((row, index) => (
                    <tr key={row._id} className="hover:bg-gray-100">
                      ...
                      <td className="border px-2 py-2">{row.leadId?.name || "-"}</td>
                      <td className="border px-2 py-2">{row.leadId?.id || "-"}</td>
                      <td className="border px-2 py-2">{row.leadId?.budget || "-"}</td>
                      <td className="border px-2 py-2">{row.leadId?.contact || "-"}</td>
                      <td className="border px-2 py-2">{row.quoteAmount || "-"}</td>
