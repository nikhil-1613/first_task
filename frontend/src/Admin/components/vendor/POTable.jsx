...
import { fetchVendorOrderByArchitect } from "../../../services/vendorOrderServices";
import { useAuth } from "../../../context/AuthContext"; // adjust path if needed

function POTable({ vendors, projects, archClients, onRowClick, onEdit, onDelete }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      if (user?.role === "architect") {
        console.log("✅ User is architect. Fetching orders for ID:", user._id);

        try {
          setLoading(true);
          const res = await fetchVendorOrderByArchitect(user._id);
          console.log("📦 Backend response for architect orders:", res);
        ...
        console.log("⚠️ User is not an architect. Skipping fetch.");
        setPurchaseOrders([]);
        setLoading(false);
      }
    };
  ...
  const getLabel = (arr, id) => arr.find((item) => item.value === id)?.label || "-";

  if (loading) {
    return <p className="text-center text-gray-500">Loading purchase orders...</p>;
  }

  return (
    <div className="bg-white p-4 rounded-xl shadow overflow-x-auto">
      {/* Debug info in UI (optional, remove later) */}
      <div className="text-xs text-gray-400 mb-2">
        Debug → Role: {user?.role || "N/A"} | ID: {user?._id || "N/A"} | Orders: {purchaseOrders.length}
      </div>
          ...
          <tbody>
            ...
            {purchaseOrders.map((po, idx) => (
              ...
              >
                ...
                <td className="p-2">{po.deliveryDate ? formatDate(po.deliveryDate) : "-"}</td>
