...
// Adjust this import to match your auth hook location
...
const ProjectSiteMeasurements = ({ projectId }) => {
  ...
  const calculateValues = (space) => {
    ...
    const openingsArea = Array.isArray(openings)
      ? openings.reduce((sum, op) => sum + Number(op.height || 0) * Number(op.width || 0), 0)
      : 0;
    const wallArea = 2 * (Number(length) + Number(breadth)) * Number(height) - openingsArea;
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setLoading(true);
      try {
        const data = await fetchSiteMeasurementsByProject(projectId);
        if (Array.isArray(data)) {
          const updated = data.map((space) => ({
            ...space,
            ...(!space.custom ? calculateValues(space) : {}),
          }));
          setSpaceList(updated);
  ...
  const updateOpening = async (spaceIndex, openingIndex, field, value) => {
    ...
    const openings = Array.isArray(updatedList[spaceIndex].openings)
      ...
      : [];
    openings[openingIndex] = { ...(openings[openingIndex] || {}), [field]: value };
  ...
  const handleDelete = async (space) => {
    ...
    try {
      ...
      setSpaceList((prev) => prev.filter((s) => (s._id || s.id) !== idToDelete));
                    ...
                    <td colSpan="11" className="bg-white py-6 px-6">
                      <div className="grid grid-cols-[1fr_1fr_1.5fr_1fr_0.8fr] gap-6 text-sm text-red-800 w-full">
                          ...
                          {["length", "breadth", "height"].map((dim) => (
                            <div key={dim} className="flex items-center gap-2">
                              ...
                              <input
                                type="number"
                                value={space[dim]}
                                onChange={(e) => updateField(index, dim, e.target.value)}
                        ...
                        <div className="space-y-4">
                            ...
                            <button
                              ...
                              onClick={() =>
                                updateField(index, "openings", [
                                  ...(Array.isArray(space.openings) ? space.openings : []),
                          ...
                          {(Array.isArray(space.openings) ? space.openings : []).map((op, opIdx) => (
                            <div key={opIdx} className="flex items-center flex-wrap gap-2">
                              <select
                                className="font-bold border p-1 rounded w-24"
                                value={op.type}
                                onChange={(e) => updateOpening(index, opIdx, "type", e.target.value)}
                              >
                              ...
                              <input
                                ...
                                value={op.height}
                                onChange={(e) => updateOpening(index, opIdx, "height", e.target.value)}
                              />
                              <input
                                type="number"
                                className="border p-1 rounded w-16"
                                value={op.width}
                                onChange={(e) => updateOpening(index, opIdx, "width", e.target.value)}
                              />
                              <button
                                className="text-red-600 font-bold"
                                onClick={() => {
                                  // remove opening
                                  const updatedOpenings = (space.openings || []).filter((_, i) => i !== opIdx);
                                  updateField(index, "openings", updatedOpenings);
                                }}
                              >
                                X
                              </button>
                          ...
                          {[
                            ...
                            <div key={key}>
                              ...
                              <input
                                ...
                                className={`w-full border p-2 rounded ${
                                  space.custom ? "bg-white" : "bg-gray-100 cursor-not-allowed"
                                }`}
                              />
                            </div>
                          ))}
                        ...
                        <div className="space-y-4">
                          <h3 className="font-bold text-red-700">Override Calculations</h3>
                          <button
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-md w-96 relative max-h-[90vh] overflow-y-auto">
            <button onClick={() => setShowModal(false)} className="absolute top-2 right-2 text-red-600 text-lg">
              <FaRegWindowClose />
            </button>
            ...
            <div className="mb-3">
              ...
              <select
                ...
                onChange={(e) => setNewRoom((s) => ({ ...s, category: e.target.value }))}
                className="w-full border p-2 rounded mb-3"
              >
              ...
              {["length", "breadth", "height"].map((field) => (
                <div key={field} className="mb-3">
                  ...
                  <input
                    type="number"
                    value={newRoom[field]}
                    onChange={(e) => setNewRoom((s) => ({ ...s, [field]: e.target.value }))}
                    className="w-full border p-2 rounded"
                  />
                </div>
              ))}
            ...
            <div className="mb-3">
              <div className="flex justify-between items-center mb-2">
                ...
                <button
                  ...
                  onClick={() =>
                    setNewRoom((s) => ({
                      ...s,
                      openings: [...(Array.isArray(s.openings) ? s.openings : []), { type: "Door", height: 0, width: 0 }],
                    }))
                  }
                >
                  <span className="text-lg font-bold">+</span> Add
              ...
              {(Array.isArray(newRoom.openings) ? newRoom.openings : []).map((op, idx) => (
                <div key={idx} className="flex items-center gap-2 mb-2">
                  ...
                  <button
                    className="text-red-600 font-bold"
                    onClick={() => {
                      const updated = newRoom.openings.filter((_, i) => i !== idx);
                      setNewRoom((s) => ({ ...s, openings: updated }));
                    }}
                  >
                    X
            ...
            <button onClick={addNewRoom} className="bg-red-700 text-white px-4 py-2 rounded mt-2 w-full">
              Add Room
            </button>
          </div>
        </div>
...
//     height: 9,
//     openings: [
//       { type: "Door", height: 7, width: 2.5 },
//       { type: "Door", height: 7, width: 2.5 },
//       { type: "Window", height: 4, width: 5 },
//     ],
//     custom: false,
//     perimeter: null,
//     floorArea: null,
...
//     height: 10,
//     openings: [
//       { type: "Door", height: 7, width: 3 },
//       { type: "Window", height: 5, width: 6 },
//     ],
//     custom: false,
//     perimeter: null,
//     floorArea: null,
...
// const ProjectSiteMeasurements = (projectId) => {
//   const [expandedRow, setExpandedRow] = useState(null);
//   const [spaceList, setSpaceList] = useState(mockData);
//   const [showModal, setShowModal] = useState(false);
//   const [unit, setUnit] = useState("ft"); // default unit
//   const [newRoom, setNewRoom] = useState({
//     id: Date.now(),
...
//   // Calculate perimeter, floor area, and wall area
//   const calculateValues = (space) => {
//     const { length, breadth, height, openings } = space;
//     const perimeter = 2 * (Number(length) + Number(breadth));
//     const floorArea = Number(length) * Number(breadth);
//     const wallArea =
//       2 * (Number(length) + Number(breadth)) * Number(height) -
//       openings.reduce((sum, op) => sum + op.height * op.width, 0);
//     return { perimeter, floorArea, wallArea };
//   };

//   // Convert units
//   const convertValue = (value) => {
//     if (unit === "m") return (value * 0.3048).toFixed(2);
//     return value;
//   };

//   // Recalculate values on initial render
//   useEffect(() => {
//     const updatedList = spaceList.map((space) =>
//       !space.custom ? { ...space, ...calculateValues(space) } : space
//     );
//     setSpaceList(updatedList);
...
//   // Add a new room
//   const addNewRoom = () => {
//     const calculatedValues = calculateValues(newRoom);
//     setSpaceList([...spaceList, { ...newRoom, ...calculatedValues }]);
//     setNewRoom({
//       id: Date.now(),
//       name: "",
//       category: "",
...
//   const updateOpening = (spaceIndex, openingIndex, field, value) => {
//     const updatedList = [...spaceList];
//     updatedList[spaceIndex].openings[openingIndex][field] = value;
...
//                 <td className="p-2">{space.height}</td>
//                 <td className="p-2">
//                   {space.openings.reduce(
//                     (sum, op) => sum + op.height * op.width,
//                     0
//                   )}{" "}
//                   sqft
//                 </td>
...
//                           {
//                             label: "Perimeter",
//                             value: convertValue(space.perimeter),
//                           },
//                           {
//                             label: "Floor Area",
//                             value: convertValue(space.floorArea),
//                           },
//                           {
//                             label: "Wall Area",
//                             value: convertValue(space.wallArea),
//                           },
//                         ].map(({ label, value }) => (
//                           <div key={label}>
//                             <label className="font-bold block">{label}</label>
...
//               {newRoom.openings.map((op, idx) => (
//                 <div key={idx} className="flex items-center gap-2 mb-2">
//                   <select
//                     className="font-bold border p-1 rounded w-24"
//                     value={op.type}
//                     onChange={(e) => {
//                       const updatedOpenings = [...newRoom.openings];
//                       updatedOpenings[idx].type = e.target.value;
//                       setNewRoom({ ...newRoom, openings: updatedOpenings });
//                     }}
//                   >
//                     <option value="Door">Door</option>
//                     <option value="Window">Window</option>
...
//                     onChange={(e) => {
//                       const updatedOpenings = [...newRoom.openings];
//                       updatedOpenings[idx].height = e.target.value;
//                       setNewRoom({ ...newRoom, openings: updatedOpenings });
//                     }}
//                   />
//                   <input
...
//                     onChange={(e) => {
//                       const updatedOpenings = [...newRoom.openings];
//                       updatedOpenings[idx].width = e.target.value;
//                       setNewRoom({ ...newRoom, openings: updatedOpenings });
//                     }}
//                   />
//                   <button
//                     className="text-red-600 font-bold"
//                     onClick={() => {
//                       const updatedOpenings = newRoom.openings.filter(
//                         (_, i) => i !== idx
//                       );
//                       setNewRoom({ ...newRoom, openings: updatedOpenings });
//                     }}
