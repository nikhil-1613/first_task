...
export default function ProjectMaterials({ projectName, rfqId }) {
  ...
  // 🔹 Load materials from the API on mount or when rfqId changes
  useEffect(() => {
    if (!rfqId) return;
    (async () => {
      try {
  ...
  // 🔹 Save new materials back to API (you can also do this inside MaterialLibraryDrawer)
  const handleAddMaterials = async (newMaterials) => {
    if (!rfqId) return;
    try {
      const updated = await addMaterialsToRFQ(rfqId, newMaterials);
      // updated could be the new list returned from backend
