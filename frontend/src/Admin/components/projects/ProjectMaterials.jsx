...
export default function ProjectMaterials({
  ...
  const [selectedRfqId, setSelectedRfqId] = useState(rfqId || null);
  ...
  // Fetch RFQs for project
  useEffect(() => {
    if (!effectiveProjectId) {
      console.log("No project ID, skipping RFQ fetch");
      return;
    }

    (async () => {
      try {
        console.log("Fetching RFQs for project:", effectiveProjectId);
        const projectRfqsRes = await fetchRFQByProject(effectiveProjectId);
        const projectRfqs = projectRfqsRes?.data || [];
        console.log("Fetched RFQs array:", projectRfqs);
        setRfqs(projectRfqs);

        // Set selected RFQ
        const initialRfqId = rfqId || projectRfqs[0]?._id || null;
  ...
  }, [effectiveProjectId, rfqId]);

  // Fetch materials for selected RFQ
  useEffect(() => {
    if (!selectedRfqId) return;

    (async () => {
      try {
        console.log("Fetching materials for RFQ:", selectedRfqId);
        let data = await getMaterialsOfRFQ(selectedRfqId);
        data = data?.data ?? data;
  ...
  }, [selectedRfqId, dispatch, effectiveProjectId, effectiveProjectName, effectiveLocation]);

  // Add new materials
  const handleAddMaterials = async (newMaterials) => {
    if (!selectedRfqId || !newMaterials || newMaterials.length === 0) return;
    ...
    try {
      ...
      let updated = await addMaterialsToRFQ(selectedRfqId, {
  ...
  const flattenedMaterials = useMemo(
    ...
    [materialGroups, effectiveProjectId, effectiveProjectName, effectiveLocation]
  );
        ...
        >
          {rfqs.map((rfq) => (
            <option key={rfq._id} value={rfq._id}>
              {`RFQ - ${new Date(rfq.date).toLocaleDateString()}`}
            </option>
          ))}
        </select>
...
//         console.log("Fetched RFQs:", projectRfqs);
//         setRfqs(projectRfqs || []);

//         const targetRfqId = rfqId || projectRfqs[0]?._id;
//         console.log("Target RFQ ID:", targetRfqId);
//         if (!targetRfqId) {
//           console.log("No RFQ ID available, skipping materials fetch");
//           return;
//         }
...
//         console.log("Flattened materials:", materialsFlat);
