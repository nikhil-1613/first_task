...
const PartyTab = () => {
  ...
  // inside PartyTab component
  const getTotalAmount = (party) => {
    if (!party) return 0;

    // If transactions exist, calculate sum
    if (party.descriptions && party.descriptions.length > 0) {
      return party.descriptions.reduce(
        (sum, txn) => sum + Number(txn.amount || 0),
        0
      );
    }

    // Otherwise, fallback to party.amount
    return Number(party.amount || 0);
  };
  ...
  const getSummaryTotals = () => {
    ...
    partyData.forEach((party) => {
      const total = getTotalAmount(party);

      // Example rules — adjust if your DB has different fields
      if (party.type === "supplier") {
        // Supplier: we owe them → toPay
        toPay += total;
      } else if (party.type === "customer") {
        // Customer: they owe us → toReceive
        toReceive += total;
      }

      // You can set conditions for advance paid/received too
      if (party.status === "advancePaid") {
        advancePaid += total;
      }
      if (party.status === "advanceReceived") {
        advanceReceived += total;
      }
    });

    return { advancePaid, toPay, toReceive, advanceReceived };
  };

  const { advancePaid, toPay, toReceive, advanceReceived } = getSummaryTotals();
              ...
              >
                ...
                <div className="text-right">
                  <p className="font-semibold">
                    {getTotalAmount(party).toLocaleString()}
                  </p>
                  {party.status && (
                    <p
                      className={`text-xs ${
            ...
            <div>
              <div className="flex justify-between items-start p-4 border-b">
                ...
                <div className="text-right flex flex-col items-end gap-1">
                  <p className="text-sm font-semibold text-gray-800">
                    {getTotalAmount(selectedParty).toLocaleString()}
              ...
              <div className="space-y-4 px-4 py-4">
                {selectedParty.descriptions &&
                selectedParty.descriptions.length > 0 ? (
                  selectedParty.descriptions.map((txn, idx) => (
