...
import { getPaymentTransactions } from "../../../services/transactionServices";
  ...
  const handleFilter = () => {
    let result = transactions.filter((txn) => {
      const matchesType = filters.type
        ? txn.transactionType === filters.type
        : true;
    ...
    toast.info(`${result.length} transaction(s) found`);
  };
      ...
      <div className="bg-white rounded-md shadow-sm divide-y">
        {filteredTransactions.length ? (
          filteredTransactions.map((txn, idx) => {
            const isPaymentIn = txn.transactionType === "PaymentIn";
            return (
              <div
                onClick={() => {
                  setSelectedTxn(txn);
                ...
                key={idx}
                className="grid sm:grid-cols-3 grid-cols-1 px-4 py-4 items-start sm:items-center text-sm hover:bg-gray-50 gap-2 cursor-pointer"
              >
                {/* Party */}
