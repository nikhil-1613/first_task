...
export default function RfqHome() {
  ...
  // Filter RFQs by project, RFQ no, supplier(s), or delivery location
  const filteredRFQs = Array.isArray(rfqDrafts)
    ? rfqDrafts.filter((rfq) => {
        const search = searchTerm.toLowerCase();
        const supplierNames = (rfq?.suppliers || [])
          .map((s) => s?.name || "")
          .join(" ")
          .toLowerCase();
    ...
    <>
      ...
      <div className="flex flex-col gap-4 p-2 sm:p-4">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-3">
          ...
          <div className="flex flex-wrap items-center gap-3 sm:gap-4 md:gap-6">
            ...
            <Button
              color="black"
              variant="custom"
              className="text-xs sm:text-sm font-medium text-gray-800 relative"
              onClick={() => setIsModalOpen(true)}
            >
              Open Material Request
              {selectedMaterials.length > 0 && (
                <span className="absolute -top-2 -right-3 bg-red-600 text-white rounded-full text-[10px] w-5 h-5 flex items-center justify-center">
            ...
            <Button
              color="red"
              className="text-white bg-red-600 hover:bg-red-700 text-xs sm:text-sm px-3 sm:px-4 py-2 rounded-lg font-semibold"
              variant="custom"
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[800px] w-full border border-gray-200 rounded-md text-xs sm:text-sm md:text-base bg-white">
            <thead className="bg-gray-400 text-black">
              <tr>
                <th className="px-2 sm:px-4 py-2 text-left">RFQ No</th>
                <th className="px-2 sm:px-4 py-2 text-left">Suppliers</th>
              ...
              {loadingRFQs ? (
                ...
                filteredRFQs.map((rfq, index) => (
                  ...
                  >
                    ...
                    <td className="px-2 sm:px-4 py-2">
                      <span className="bg-yellow-200 text-yellow-800 px-2 py-1 rounded-md text-xs sm:text-sm font-medium">
                        {rfq.status || "Draft"}
                      </span>
                    </td>
                  </tr>
...
//                     <td className="px-2 sm:px-4 py-2">
//                       {rfq?.suppliers?.length > 0
//                         ? rfq.suppliers.map((s) => s?.name).filter(Boolean).join(", ")
//                         : "--"}
//                     </td>
//                     <td className="px-2 sm:px-4 py-2">
//                       {rfq?.materials?.length || 0}
